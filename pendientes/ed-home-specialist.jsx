...
function EdHomeManager({ theme = 'light' }) {
  ...
  return (
    ...
    <div className="ed-root">
      ...
      <main className="ed-main">
        <EdHeader t={t}/>
        <div className="ed-eye">Jueves 23 abril · Equipo CO</div>
        <h1 className="ed-h1">Tu equipo cargó <em>11 tareas</em><br/>de alta prioridad esta semana.</h1>
        <p className="ed-lede">
          <b>Juan Camilo</b> está sobrecargado con <span className="crit">2 tareas vencidas</span>;
          <b> Ana Bravo</b> tiene riesgo en una demanda en Medellín. La utilización del equipo es del <b>65%</b> —
          <span className="good"> dentro de rango saludable</span>, pero hay que reasignar las dos vencidas hoy.
        </p>

        <div className="mgr-hero">
          <div className="mgr-stat"><div className="mgr-stat-lbl">Tareas vencidas</div><div className="mgr-stat-num crit">04</div><div className="mgr-stat-ctx">en 3 personas</div></div>
          ...
          <div className="mgr-stat"><div className="mgr-stat-lbl">Carga equipo</div><div className="mgr-stat-num">65<span style={{fontSize:18,color:t.dim}}>%</span></div><div className="mgr-stat-ctx">11/17 capacidad</div></div>
        ...
        <section className="ed-section">
          <div className="ed-h2">— Pide tu atención, ahora</div>
          {[
            { rank: 1, id: 'T-142', who: 'Juan Camilo', what: 'Demanda laboral Bogotá vencida hace 3 días', why: <><span className="crit">Bloqueada por RR.HH.</span> sin respuesta. Reasignar o escalar a director RR.HH.</> },
            { rank: 2, id: 'T-115', who: 'Ana Bravo', what: 'Demanda consumidor Medellín — vence en 1d', why: <>Riesgo legal alto. <b>Falta una declaración del cliente</b> para responder.</> },
            { rank: 3, id: 'T-138', who: 'Juan Camilo', what: 'Contrato proveedor delivery — venció ayer', why: <>En curso. JC reportó que <b>la cláusula 7.2 necesita acuerdo comercial</b>.</> },
            { rank: 4, id: 'T-125', who: 'Juan Manuel', what: 'Respuesta SIC — vencida hace 4d', why: <><span className="crit">Sin actualización en 5 días.</span> Pedirle status hoy.</> },
          ].map(it => (
            <div key={it.id} className="mgr-att-row">
              <div className="mgr-att-rank crit">{String(it.rank).padStart(2,'0')}</div>
              <div>
                <div className="mgr-att-meta">{it.id} · {it.who}</div>
                <div className="mgr-att-name">{it.what}</div>
                <div className="mgr-att-narr">{it.why}</div>
              </div>
              <button className="ed-btn">Ver</button>
              <button className="ed-btn primary">Reasignar</button>
            </div>
          ))}
        </section>
...
function EdHomeHQ({ theme = 'light' }) {
  ...
  return (
    ...
    <div className="ed-root">
      ...
      <main className="ed-main">
        <EdHeader t={t}/>
        <div className="ed-eye">Jueves 23 abril · Visión global · 10 países</div>
        <h1 className="ed-h1">Operación legal global,<br/><em>vista de un día</em>.</h1>
        <p className="ed-lede">
          <b>Brasil</b> requiere atención: <span className="crit">7 vencidas</span> y SLA cayó a 79% (-6pp este mes).
          <b> Colombia y México</b> están en franja saludable. Total de tareas abiertas: <b>217</b>, con
          <span className="good"> 89% SLA promedio LATAM</span>.
        </p>

        <div className="hq-hero">
          <div className="hq-stat"><div className="hq-stat-lbl">Total abiertas</div><div className="hq-stat-num">217</div><div className="hq-stat-ctx">+12 vs sem pasada</div></div>
          <div className="hq-stat"><div className="hq-stat-lbl">Vencidas global</div><div className="hq-stat-num" style={{color:t.critical}}>16</div><div className="hq-stat-ctx">7 en BR · 4 en CO</div></div>
          ...
          <div className="hq-stat"><div className="hq-stat-lbl">Países en riesgo</div><div className="hq-stat-num" style={{color:t.warn}}>02</div><div className="hq-stat-ctx">BR · CR</div></div>
        </div>
