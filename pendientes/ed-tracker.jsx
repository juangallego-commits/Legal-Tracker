...
function EdTracker({ theme = 'light', role = 'manager', selectedId = null }) {
  ...
  const fmtEta = (d) => {
    if (d < 0) return { text: `+${Math.abs(d)}d · cierre pendiente`, cls: 'warn' };
  ...
  // Mock: assign confidentiality level per task id (deterministic for demo)
  const confByTaskId = {
    'T-142': 'restringido',
    'T-138': 'estandar',
    'T-125': 'estandar',
  ...
  const getConf = (id) => ED_CONF_LEVELS[confByTaskId[id] || 'estandar'];
    ...
    <div className={`ed-root ${sel ? 'with-panel' : ''}`}>
      ...
      <main className="ed-main">
        ...
        <p className="ed-lede" style={{marginBottom:28}}>
          {role === 'specialist' && <>Tus 7 tareas activas. Cliquea cualquiera para ver detalle, historial y siguiente acción sugerida.</>}
          {role === 'manager' && <>11 tareas activas en tu equipo CO. Cliquea cualquier fila para ver detalle, historial y opciones de seguimiento.</>}
          {role === 'hq' && <>20 tareas activas en LATAM. Filtrá por país o líder para enfocarte en una operación.</>}
        </p>

        <div className="tk-toolbar">
          <div className="tk-filter active">Todas <span className="count">{visibleTasks.length}</span></div>
          <div className="tk-filter">Pendientes de cierre <span className="count" style={{color:t.warn}}>{visibleTasks.filter(t=>t.deadline<0).length}</span></div>
          <div className="tk-filter">Hoy <span className="count">{visibleTasks.filter(t=>t.deadline===0).length}</span></div>
          <div className="tk-filter">En espera <span className="count">{visibleTasks.filter(t=>t.status==='Bloqueado').length}</span></div>
        ...
        <table className="tk-table">
          <thead>
            <tr>
              <th style={{width:28,textAlign:'center'}}>🔒</th><th>ID</th><th>Tarea</th><th>{role==='specialist'?'Proyecto':'Responsable'}</th><th>Prioridad</th><th>Estado</th><th>ETA</th>
            </tr>
          </thead>
          <tbody>
            {visibleTasks.map(r => {
              ...
              return (
                <tr key={r.id} className={`tk-row ${r.id===sel?.id?'selected':''}`}>
                  <td style={{textAlign:'center'}}><span style={{display:'inline-block',width:9,height:9,borderRadius:'50%',background:getConf(r.id).dot}} title={getConf(r.id).label}></span></td>
      ...
      {sel && (
        <aside className="tk-panel">
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:16,padding:'8px 12px',background:getConf(sel.id).band,borderRadius:4}}>
            <span style={{width:10,height:10,borderRadius:'50%',background:getConf(sel.id).dot}}></span>
            <span style={{fontSize:11,fontWeight:800,color:getConf(sel.id).bandInk,textTransform:'uppercase',letterSpacing:.6}}>{getConf(sel.id).label}</span>
            <span style={{marginLeft:'auto',fontSize:10.5,color:getConf(sel.id).bandInk,opacity:.75}}>
              {confByTaskId[sel.id] === 'confidencial' ? 'Acceso · 4 personas ▾' : confByTaskId[sel.id] === 'restringido' ? 'Acceso · equipo + manager ▾' : 'Acceso · todo el equipo CO'}
            </span>
          </div>
          ...
          <div className="tk-panel-section">
            ...
            <div className="tk-panel-block">
              <b>En espera de RR.HH. desde hace 4 días.</b> Pidieron el contrato del demandante para responder.
              JC ya envió dos seguimientos (16 abr y 19 abr).<br/><br/>
              <b>Acción sugerida:</b> escalar al Director de RR.HH. con un mensaje cordial, o pedir apoyo al manager para destrabarla.
            </div>
          </div>
          ...
          <div className="tk-panel-section">
            <div className="tk-panel-grid">
              ...
              <div className="tk-panel-cell"><div className="tk-panel-cell-lbl">Deadline</div><div className="tk-panel-cell-val" style={{color:t.warn}}>20 abr · +3d sin cierre</div></div>
              <div className="tk-panel-cell"><div className="tk-panel-cell-lbl">SLA</div><div className="tk-panel-cell-val">2 días</div></div>
              <div className="tk-panel-cell"><div className="tk-panel-cell-lbl">Nivel de riesgo</div><div className="tk-panel-cell-val"><span className="ed-pill alta" style={{verticalAlign:'middle'}}>Alto</span></div></div>
          ...
          <div className="tk-panel-section">
            ...
            <div className="tk-timeline">
              <div className="tk-tl-item">
                ...
                <div className="tk-tl-what">El sistema marcó esta tarea como <b>pendiente de cierre</b>. Sin movimiento en 4 días.</div>
              </div>
              <div className="tk-tl-item">
                <div className="tk-tl-dot"/>
                <div className="tk-tl-when">19 abr · 16:30</div>
                <div className="tk-tl-what"><b>Juan Camilo:</b> "Reenvío email a RR.HH., todavía sin respuesta."</div>
              </div>
              <div className="tk-tl-item">
                <div className="tk-tl-dot"/>
                <div className="tk-tl-when">16 abr · 11:02</div>
                <div className="tk-tl-what">Estado cambió a <b>en espera</b> · razón: dependencia externa con RR.HH.</div>
              </div>
              <div className="tk-tl-item">
          ...
          <div className="tk-actions">
            <button className="ed-btn primary">Escalar al Director RR.HH.</button>
            <button className="ed-btn">Pedir apoyo al manager</button>
            <button className="ed-btn">Comentar en la tarea</button>
          </div>
        </aside>
      )}
    </div>
