...
// Antes de entrar al tracker denso, un mapa por urgencia/proyecto/tipo
// ═══════════════════════════════════════════════════════════════
...
function EdAgrupadas({ theme = 'light' }) {
  ...
  const rawCSS = edBaseCSS(t) + `
    ...
    .ag-card-action .crit { color:${t.critical}; font-weight:700; }
  ...
  const buckets = [
    {
      key: 'overdue', tone: 'crit', title: 'Vencidas',
      narr: <>Estas <span className="crit">venían con SLA Alta y se pasaron</span>. Una está bloqueada por RR.HH., otra en curso. <b>Acción primaria: desbloquear o escalar.</b></>,
      tasks: ED_BUCKETS_MINE.overdue,
    },
    {
      key: 'today', tone: 'warn', title: 'Vencen hoy',
      narr: <>Una sola, pero crítica: <b>policy de data retention</b>, esperando firma del Privacy Officer. <span className="crit">Sin firma, no se cierra.</span></>,
      tasks: ED_BUCKETS_MINE.today,
    },
    {
      key: 'week', tone: '', title: 'Esta semana',
      narr: <>3 tareas con buen margen. Empieza con la <b>opinión regulatoria de método de pago</b> que tiene más alcance.</>,
      tasks: ED_BUCKETS_MINE.thisWeek,
    },
    {
      key: 'later', tone: '', title: 'Más adelante',
      narr: <>2 tareas con SLA holgado. <b>No te apures por estas hoy.</b></>,
  ...
  return (
    ...
    <div className="ed-root">
      ...
      <main className="ed-main">
        <EdHeader t={t}/>
        <div className="ed-eye">Mis tareas — agrupadas por urgencia</div>
        <h1 className="ed-h1">Siete tareas, cuatro <em>franjas</em><br/>de tiempo distintas.</h1>
        <p className="ed-lede">
          Antes de meterte a la tabla densa, esta es la lectura rápida: dónde está cada cosa, qué pide tu atención
          primero y por qué. <b>Empezá por las dos vencidas</b>; las de "más adelante" pueden esperar.
        </p>

        <div className="ag-tabs">
          <div className="ag-tab active">Por urgencia</div>
                ...
                return (
                  <div key={task.id} className="ag-card">
                    ...
                    <div className="ag-card-action">
                      {task.blocked ? <><span className="crit">Bloqueada por {task.blocked}.</span> {task.accionable}</> : task.accionable}
                    </div>
