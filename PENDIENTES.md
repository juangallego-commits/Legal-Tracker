...
| 12 | **Activar el Gmail Add-on para todo Legal** | Publicarlo como **app privada en el Workspace Marketplace** y que IT lo instale para la OU/grupo de Legal. **Runbook paso a paso: [`docs/GMAIL-ADDON-MARKETPLACE.md`](docs/GMAIL-ADDON-MARKETPLACE.md).** | Decidido: Opción B (Marketplace), no test deployments (no escala). El `clasp push` (CI) sube el **código**, pero Gmail no lo muestra hasta que exista un deployment de add-on publicado e instalado por el admin. Para que los cambios futuros del add-on se auto-actualicen, setear el secret `ADDON_DEPLOYMENT_ID` (ver runbook). Cada persona debe estar en la hoja `Equipos` y consentir los scopes la 1ª vez. |
